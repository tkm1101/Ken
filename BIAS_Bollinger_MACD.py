...
while True:
    ...
    diff=df_midclose['MidClose'].ewm(span=12).mean()-df_midclose['MidClose'].ewm(span=26).mean()
    dea=diff.ewm(span=9).mean()
    
    
    #Start placing trade
    if invested()== 'empty':
        if diff[499] > 0 and dea[499] >0 and bias_formal[499] < xll[499]:
            oanda.create_order(account_id=account_number,
                              instrument='UK100_GBP',
                              units=1,
                              side='buy',
                              type='market')
        elif diff[499] < 0 and dea[499] < 0 and bias_formal[499]> shh[499]:
            oanda.create_order(account_id=account_number,
                              instrument='UK100_GBP',
                              units=1,
                              side='sell',
